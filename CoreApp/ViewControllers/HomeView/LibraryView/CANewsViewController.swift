...
class CANewsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, FilterProtocol {
    ...
    func initialSetup() {
        ...
        //Add Observer
        NotificationCenter.default.addObserver(self, selector: #selector(searchApi(notification:)), name: NSNotification.Name("newsNotification"), object: nil)
...
//        NotificationCenter.default.addObserver(self, selector: #selector(callNewsApi(notification:)), name: NSNotification.Name("newsApiCall"), object: nil)

        callApiForNewsData(searchText: "", noFilter: false)
    }
...
//    @objc func callNewsApi(notification: NSNotification) {
//        let dict = notification.object as! NSDictionary
//        let str = "\(dict["searchValue"] as! String)".trimmingCharacters(in: .whitespaces).lowercased()
//        callApiForNewsData(searchText: str, noFilter: false)
//    }

    @objc func searchApi(notification: NSNotification) {
        let dict = notification.object as! NSDictionary
        let str = "\(dict["searchValue"] as! String)".trimmingCharacters(in: .whitespaces).lowercased()
        callApiForNewsData(searchText: str, noFilter: true)
    }
...
//                }
    ...
    func filterList(strValue: String) {
        callApiForNewsData(searchText: strValue, noFilter: false)
    }


    //MARK: Call API
    func callApiForNewsData(searchText: String, noFilter: Bool)
    {
        var dict = Dictionary<String, AnyObject>()
        var val = Int()
        val = NSUSERDEFAULT.integer(forKey: "newsFilter")
...
//        dict[pDownload] = "1" as AnyObject
        ...
        CAServiceHelper.request(dict, method: .get, apiName: pNews, hudType: .simple) { (result, error, status) in
