...
- (void)initValues
{
    ...
    _menuHeight                           = 34.0;
    _menuMargin                           = 0.0;
    _menuItemWidth                        = UIScreen.mainScreen.bounds.size.width/5;
    _selectionIndicatorHeight             = 5.0;
    ...
    _menuItemWidthBasedOnTitleTextWidth = YES;
    _useMenuLikeSegmentedControl        = YES;
    _centerMenuItems                    = YES;
...
- (void)setUpUserInterface
{
    ...
    _controllerScrollView.frame = CGRectMake(0.0, _menuHeight, UIScreen.mainScreen.bounds.size.width, self.view.frame.size.height);
    ...
    _menuScrollView.frame = CGRectMake(0.0, 0.0, UIScreen.mainScreen.bounds.size.width, _menuHeight);
...
//        _menuScrollView.contentSize = CGSizeMake((_menuItemWidth + _menuMargin) * (CGFloat)_controllerArray.count + _menuMargin, _menuHeight);
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (controller.title != nil) {
            menuItemView.strViewTitle = controller.title;
            [menuItemView setTitleText:menuItemView.strViewTitle];
        } else {
...
- (void)moveToPage:(NSInteger)index
{
    if (index >= 0 && index < _controllerArray.count) {
        ...
        // Move controller scroll view when tapping menu item
        double duration = (double)(_scrollAnimationDurationOnMenuItemTap) / (double)(5000);
...
//#pragma mark - Language Observer Methods
//
//-(void)languageChanged{
//
//    for (MenuItemView *itemViewTemp in _menuScrollView.subviews) {
//
//        if ([itemViewTemp isKindOfClass:[MenuItemView class]]) {
//
//            itemViewTemp.titleLabel.text = itemViewTemp.strViewTitle;
//        }
//    }
//    [[NSNotificationCenter defaultCenter] postNotificationName:KNOTIFICATION_RELOAD_TABLE object:nil];
//
//}
